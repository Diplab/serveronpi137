...
public class RpiSmoke {
	
	/************************Hardware Related Macros************************************/
	private static final int MQ_PIN = 3;     //define which analog input channel you are going to use
	private static final double RL_VALUE  = 5;     //define the load resistance on the board, in kilo ohms
	private static final double RO_CLEAN_AIR_FACTOR    = 9.83;  //RO_CLEAR_AIR_FACTOR=(Sensor resistance in clean air)/RO,
	...
	private static final int GAS_CO       = 1;
	private static final int GAS_SMOKE    = 2;
	 
	/*****************************Globals***********************************************/
	private static double[] LPGCurve  =  {2.3,0.21,-0.47};   //two points are taken from the curve. 
	                                                    //with these two points, a line is formed which is "approximately equivalent"
	                                                    //to the original curve. 
	                                                    //data format:{ x, y, slope}; point1: (lg200, 0.21), point2: (lg10000, -0.59) 
	private static double[] COCurve  =  {2.3,0.72,-0.34};    //two points are taken from the curve. 
	                                                    //with these two points, a line is formed which is "approximately equivalent" 
	                                                    //to the original curve.
	                                                    //data format:{ x, y, slope}; point1: (lg200, 0.72), point2: (lg10000,  0.15) 
	private static double[] SmokeCurve ={2.3,0.53,-0.44};    //two points are taken from the curve. 
	                                                    //with these two points, a line is formed which is "approximately equivalent" 
	                                                    //to the original curve.
	...
	public static void main(String[] args) throws InterruptedException {
		...
		while (true) {
			System.out.format("smoke = %-10.3f ppm%n", RpiSmoke.get());
			Thread.sleep(1000);

		}
	}

	static double readadc_MQ2(int ch) {
	...
	************************************************************************************/ 
	static double MQRead(int mq_pin)
	{
	  ...
	  for (i=0;i<READ_SAMPLE_TIMES;i++) {
	    rs += MQResistanceCalculation(readadc_MQ2(mq_pin));
	    try {
	...
	************************************************************************************/ 
	...
	{
	  if ( gas_id == GAS_LPG ) {
	     return MQGetPpm(rs_ro_ratio,LPGCurve);
	  } else if ( gas_id == GAS_CO ) {
	     return MQGetPpm(rs_ro_ratio,COCurve);
	  } else if ( gas_id == GAS_SMOKE ) {
	     return MQGetPpm(rs_ro_ratio,SmokeCurve);
	  }    
